...
def parse_log(log_path, fold_id=1):
    pattern = re.compile(
        r"Fold\s+(\d+),\s+Epoch\s+\[(\d+)/(\d+)\],.*?Val Loss:\s*([0-9.]+),.*?LR:\s*([0-9.eE+-]+)"
    )
    ...
    return np.array(epochs), np.array(val_loss), np.array(lrs)
...
def plot_curve(log_path, out_path, fold_id=1, smooth_window=5):
    epochs, val_loss, lrs = parse_log(log_path, fold_id=fold_id)
    val_loss_smooth = moving_average(val_loss, w=smooth_window)
    ...
    ax1.plot(epochs, val_loss, color="#1f77b4", alpha=0.35, linewidth=1.2, label="Val Loss")
    ax1.plot(epochs, val_loss_smooth, color="#1f77b4", linewidth=2.0, label=f"Val Loss MA({smooth_window})")
    ax1.scatter([best_epoch], [best_val], color="#1f77b4", s=40, zorder=5)
    ax1.set_xlabel("Epoch")
    ax1.set_ylabel("Val Loss", color="#1f77b4")
    ...
    print(f"最优 Val Loss: {best_val:.6f}, Epoch: {best_epoch}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default="lr_val_loss_curve.png", help="输出图片路径")
